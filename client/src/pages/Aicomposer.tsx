    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        <div className="relative group mt-12">
          <textarea
            className="w-full px-6 py-6 bg-white border border-slate-300 rounded-xl text-slate-900 placeholder-slate-400 outline-none focus:border-slate-400 transition resize-none h-44"
      ...
      {activeScheduler && (
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            {/* Modal Header */}
            <div className="flex items-center justify-between px-8 py-4 border-b border-slate-100 bg-slate-50/30 shrink-0">
            ...
            {/* Fixed Layout Footer Panels */}
            <div className="space-y-6 p-8 border-t border-slate-100 bg-slate-50/50 shrink-0">
              <div>
                <label className="block text-xs text-slate-600 uppercase tracking-widest mb-3 font-medium">
                  Select Channels
                </label>
                  ...
                  {PLATFORMS.map((p) => {
                    ...
                    return (
                      <button
                        ...
                        className={`p-2.5 rounded-md border text-xs transition-all duration-150 ${
                          active
              ...
              {/* Grid Wrapper Container for Time Inputs */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative">
                  <CalendarIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                  <input
                    type="date"
                    className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-md text-slate-900 text-sm focus:outline-none transition-all"
                    value={scheduledDate}
                ...
                <div className="relative">
                  <ClockIcon className="size-4 absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                  <input
                    type="time"
                    className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-md text-slate-900 text-sm focus:outline-none transition-all"
                    value={scheduledTime}
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-900 text-white hover:bg-slate-800 font-medium transition-all"
              >
                {scheduling ? (
                  <Loader2Icon className="size-4 animate-spin" />
                ) : (
