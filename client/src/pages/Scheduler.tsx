...
const Scheduler = () => {
  ...
  const fetchPosts = async () => {
    ...
    setIsFetching(true);
    try {
      console.log("🔵 [FRONTEND] Fetching posts...");
      const { data } = await api.get("/posts");
      console.log(`✅ [FRONTEND] Fetched ${data.length} posts`);
      setPosts(data);
    } catch (error: any) {
  ...
  useEffect(() => {
    fetchPosts();
    // ❌ REMOVED the setInterval - no more infinite loops!
  ...
  const handleSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("❌ [FRONTEND] Error scheduling:", error);
      toast.error(error?.response?.data?.message || error.message);
    } finally {
      setLoading(false);
    }
              ...
              {mediaFile ? (
                <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 p-1.5 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full transition-colors"
                  >
